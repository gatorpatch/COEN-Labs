...
void insert(char *cName,int cSize) {
    ...
    add = (RESERV*)malloc(sizeof(RESERV));
    //lock the list
    ...
    for(i = currentList;i >= 0;i--) {
        ...
        while(current != NULL) {
            if(current->size <= sFind) {
                ...
                RESERV *serCurrent = NULL;
                serCurrent = (RESERV*)malloc(sizeof(RESERV));
...
int check_duplicate(char name[LENGTH]) {
    RESERV *current;
    current = (RESERV*)malloc(sizeof(RESERV));
    int i;
    pthread_mutex_lock(&lock);
...
void changeSize() {
    ...
    RESERV *prev;
    RESERV *new;
    new = (RESERV*)malloc(sizeof(RESERV));
